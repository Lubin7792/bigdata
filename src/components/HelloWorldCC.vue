  ...
  <div class="hello">
    ...
    <div class="skorke">
      <div class="svgvan">
       ...
       <beltPopup  :stackerDatas=this.machinePosition :popupDatas="this.popupData"></beltPopup>
      </div>
...
export default {
  ...
  methods: {
    timename(e){
      let statetime = new Date(e.endtime / 1000);
      console.log(e,e.device_name,statetime.getHours(), statetime.getMinutes(),e.status )
    },
    showPopup(value){
    },
    ...
    init(datas) {
      let svgdata=[],popup=[],machine=[],sW=210,sT=10,esW=170,esT=30,csW=130,csT=60,cesW=100,cesT=20;
      datas.map(item => {
        let dataname;
          ...
          if((item.status=="gz")&&(item.device_belt_name_simplify.slice(0,1)=="B")){
            dataname = item.device_belt_name_simplify; 
               for (const key in positionData.belt) {
              if(key == dataname){
                let obj={
          ...
          // 装船机设备
          if((item.status=="gz")&&(item.device_belt_name_simplify.slice(0,2)=="SL")){
            dataname = item.device_belt_name_simplify; 
          ...
          // 翻车机
          if((item.status=="gz")&&(item.device_belt_name_simplify.slice(0,2)=="CD")){
            dataname = item.device_belt_name_simplify; 
          ...
          // 堆取料机
          if((item.status=="gz")&&(item.device_belt_name_simplify.slice(0,1)=="R")){
            dataname = item.device_belt_name_simplify; 
            let obj={
                  name:dataname,
                   x:Number(this.stackerPosition(dataname).left.replace(/px/g,''))+sW,
                  y:Number(this.stackerPosition(dataname).top.replace(/px/g,''))-sT,
                  ex:Number(this.stackerPosition(dataname).left.replace(/px/g,''))+esW,
                  ey:Number(this.stackerPosition(dataname).top.replace(/px/g,''))+esT
                };
              machine.push(obj);
          }
          if((item.status=="gz")&&(item.device_belt_name_simplify.slice(0,2)=="SR")){
            dataname = item.device_belt_name_simplify; 
            let obj={
                  name:dataname,
                   x:Number(this.stackerPosition(dataname).left.replace(/px/g,''))+sW,
          ...
          if((item.status=="gz")&&(item.device_belt_name_simplify.slice(0,1)=="S")&&(item.device_belt_name_simplify.slice(1,2)!="L")){
            dataname = item.device_belt_name_simplify; 
            let obj={
                  name:dataname,
                  x:Number(this.stackerPosition(dataname).left.replace(/px/g,''))+sW,
